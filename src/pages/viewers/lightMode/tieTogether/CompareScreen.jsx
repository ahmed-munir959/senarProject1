      ...
      <div className="bg-white rounded-lg w-full max-w-2xl p-6 absolute bottom-0 md:relative md:bottom-auto md:w-[864px] md:h-[454px] md:mx-auto md:border md:border-gray-200 md:overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 md:grid-cols-3 md:w-full md:max-w-[750px] md:h-[82px] md:mx-auto md:gap-6">
          {selectedStudios.slice(0, 2).map((studio) => (
            <div
              key={studio.id}
              className="border border-gray-200 rounded-lg p-4 relative md:w-[200px] md:h-[82px] md:p-5 md:rounded-xl md:border-gray-300 md:flex md:flex-row md:gap-2.5"
            >
          ...
          <div className="col-span-2 md:col-span-1 border border-gray-300 rounded-lg p-4 flex items-center justify-center h-full w-full md:max-w-[214px] md:h-[82px] md:p-5 md:rounded-xl">
            <button
              onClick={() => {
                if (studioToCompare) {
                  const updated =
              ...
              className="w-full h-full text-gray-400 text-sm font-medium leading-[140%] hover:text-purple-500 transition-colors"
            >
              +Add to compare
            </button>
          </div>
