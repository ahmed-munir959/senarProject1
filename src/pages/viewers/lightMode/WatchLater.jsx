...
const WatchLater = () => {
  // Section categories - only keeping Watch Later as requested
  const sections = [
    {
      title: "Watch Later",
      movies: Array(12).fill(null), // Expanded to 12 to have more items to display
    },
  ];
  ...
  const getMovieImages = (sectionTitle, index) => {
    // Using modulo to cycle through available images
    const imageIndex = index % 4;

    switch (sectionTitle) {
      case "Watch Later":
        return [thriller1, thriller2, thriller3, thriller4][imageIndex];
      default:
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <nav className="mt-6">
              <div className="px-6">
                ...
                <a
                  href="#"
                  className="flex items-center py-3 bg-white rounded-md"
                  onClick={() => setActiveItem("Watch Later")}
                >
                  <svg
                    className="w-5 h-5 mr-3 text-purple-800"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
            ...
            <div className="p-4 md:p-6">
              {sections.map((section, sectionIndex) => (
                <div key={sectionIndex} className="mb-6 md:mb-10">
                  <h2 className="font-sans font-medium leading-[1.4] tracking-normal text-2xl text-gray-800 mb-3 md:mb-4">
                    {section.title}
                  </h2>
                  {/* Modified grid to display 3 images per row with specific dimensions */}
                    ...
                    {section.movies.map((_, movieIndex) => {
                      ...
                      return (
                        <div key={movieIndex} className="relative">
                          <img
                            src={imgSrc}
                            className="w-full h-48 md:h-50 object-cover rounded-xl md:rounded-2xl"
